...
summary: 웹앱의 작동 원리부터 SSR 방식을 적용한 현대적 접근까지
---
...
<div className="flex flex-col sm:flex-row justify-center p-0">
  <Image src="/static/images/levelup/cover_front.png" width="336" height="480" className="w-auto sm:w-auto"/>
  <Image src="/static/images/levelup/cover_middle.png" width="76" height="480" className="hidden sm:block w-auto sm:w-auto"/>
  <Image src="/static/images/levelup/cover_back.png" width="336" height="480" className="hidden sm:block w-auto sm:w-auto"/>
</div>


- 온라인 서점 구매: [예스24](https://www.yes24.com/Product/Goods/126689601), [교보문고](https://product.kyobobook.co.kr/detail/S000213362009), [알라딘](https://www.aladin.co.kr/shop/wproduct.aspx?ISBN=K922931869&start=pnaver_02), [인터파크](http://inpk.kr/s0gB), [네이버](https://search.shopping.naver.com/book/catalog/47914364633?query=next.js&NaPm=ct%3Dlwq2046o%7Cci%3Dd710a4ca10abfaf597b712254a8ae6a27424ba6b%7Ctr%3Dboksl%7Csn%3D95694%7Chk%3D0fdbb5df1d190857a7cf4027e212c1b90a7b7d43)
- 소스코드 다운로드: [Github 링크](https://github.com/bjpublic/react_programming_with_Next.js)
...
<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
  <Card href="/levelup/book/feedback">
    ### 도서 내용 피드백
    오타, 내용 추가, 소스 코드 설명 등 피드백을 남겨 주세요.
  </Card>

  <Card href="/levelup/book/discussion">
...
> 성능 향상, 새로운 스트리밍 텍스트 프로토콜 도입, `useCompletion` 훅 추가 등의 새로운 기능이 추가되었으며 기존의 `StreamingTextResponse` API가 새로운 프로토콜로 대체되었습니다. 버전 3.1에서는 클라이언트와 서버 간의 데이터 스트리밍 기능이 개선되고 다양한 AI 모델 제공자에 대한 지원이 강화되었습니다. 버전 3.2에서는 버그 수정 및 안정성 개선, 성능 최적화, 새로운 기능의 도입이 이루어졌습니다.

그래서 여기 웹 페이지를 통해 아래 소스 코드와 블로그에서 추가 정보를 제공합니다.

<Callout type="warning" icon="날씨 AI 챗봇 v2">

업데이트된 버전은 최신 버셀 AI SDK를 사용하고 OpenAI의 Chat Completion과 Assistants를 이용하여 구현된 예제를 설명합니다.

- 소스 코드: https://github.com/Eirene-dev/levelup-weather-chat-v2
- 상세 코드 설명: https://reactnext-central.xyz/example/weather-ai-chatbot
- 블로그
...
### 「11.3 버셀 Postgres 데이터베이스 생성 및 환경 변수 설정」 Neon Postgres 데이터베이스 생성으로 변환
> 기존 버셀 Postgres가 Neon Postgres로 전환되었습니다.

책에서 다룬 `버셀 Postgres` 관련 챕터나 코드는 **Neon Postgres를 위한 설정**으로 간단히 대체 가능하며 **접속 정보 변경** 이외의 구조나 문법은 거의 동일합니다.
오히려 Neon 측 대시보드와 관리 기능이 더 세분화·전문화되었으므로 **서버리스 Postgres** 환경을 좀 더 편리하고 안정적으로 사용할 수 있게 되었습니다.

1. **인프라 제공 주체 변경**  
   - 예전엔 Vercel이 직접 Postgres를 관리했지만, 이제는 **Neon**이라는 전문 서비스가 Postgres 인프라를 호스팅합니다.  
   - 기존에 버셀 Postgres를 사용하던 분들은 **자동 전환**되므로 큰 코드 수정 없이 새 환경을 쓸 수 있습니다.

2. **데이터베이스 생성 및 관리**  
   - Vercel 대시보드에서 **Neon Postgres**를 생성한 뒤, 더 자세한 DB 관리는 **Neon Dashboard**에서 이루어집니다.  
   - ‘버셀 Postgres 데이터베이스 생성’이라는 용어가 책에 나와 있다면, 지금은 **Neon Postgres 생성**으로 바뀌었다고 보면 됩니다.

3. **환경 변수 설정**  
   - 책에서 안내하던 `DATABASE_URL` 기반 설정은 그대로 유효합니다.  
   - 다만 연결 정보가 Vercel Postgres가 아닌 **Neon Postgres** 주소로 바뀌며, 책의 코드 예시와 마찬가지로 `.env.local` 혹은 Vercel 대시보드의 Environment Variables에 저장해 사용하면 됩니다.

4. **쿼리 실행 및 테이블 관리**  
   - 책에서 Vercel 대시보드에서 SQL 쿼리를 실행하거나 테이블을 만드는 과정을 설명했다면, 이제는 **Neon Dashboard**(또는 제공되는 SQL 에디터)를 통해 진행합니다.  
   - 사용 방법은 유사하지만, DB 관리 인터페이스가 **Neon** 쪽으로 옮겨갔다고 이해하면 됩니다.

5. **트랜잭션 및 서버리스 특성**  
   - 서버리스 Postgres라는 점은 동일하지만, Neon은 **자동 확장**, **Branching** 등 더 발전된 기능을 제공합니다.  
   - 책의 예제 코드를 그대로 사용하되, DB 연결 부분만 새로 생성된 Neon Postgres 정보로 교체하면 됩니다.

<Callout type="warning" icon="Neon Postgres 상세 설명">
- 책 새로운 본문: [Neon Postgres 「11.3 버셀 Postgres 데이터베이스 생성 및 환경 변수 설정」](/levelup/book/neon-postgres)
- 관련 블로그: [Neon Postgres: 달라진 Vercel 환경에서의 완성도 높은 업그레이드](/blog/web-dev/vercel-neon-postgres)
</Callout>
