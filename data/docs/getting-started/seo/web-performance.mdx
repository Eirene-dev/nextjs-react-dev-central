...
summary: Google이 웹 사용자 경험을 측정하기 위해 제시한 핵심 웹 척도(LCP, FID, CLS)에 대한 개념, SEO와 UX에 미치는 영향, 그리고 Lighthouse의 척도 가중치에 관한 정보를 제공합니다.
---
...
<span className="text-blue-600 font-bold">웹 척도(Web Vitals)</span>는 
<span className="font-semibold bg-yellow-100">Google이 웹에서 최종 사용자 페이지 경험을 측정하기 위해 제공하는 통합 지침 및 지표</span>의 선도적인 이니셔티브입니다.

핵심 웹 척도는 웹 척도의 하위 집합이며 현재는 <span className="font-bold underline">로딩</span>, 
...
핵심 웹 척도 각 지표에서 점수가 낮은 웹사이트는 Google이 <span className="font-bold underline">검색 알고리즘의 순위 요인으로 핵심 웹 척도를 사용</span>하기 시작함에 따라 검색 엔진 순위에 영향을 줄 수 있습니다. 나쁜 척도는 웹 트래픽 및 비즈니스에 영향을 줄 수 있습니다.

이 강의에서는 다음과 같은 내용을 배울 수 있습니다:

- 핵심 웹 척도에 대한 간략한 배경.
- 핵심 웹 척도의 SEO 및 UX 영향과 웹사이트에 미치는 영향.
- 개발 과정에서 핵심 웹 척도에 관심을 가져야 하는 이유와 측정 방법.
- Next.js를 사용하여 핵심 웹 척도를 향상시키고 변화를 모니터링하는 방법.

## 핵심 웹 척도 (Web Vitals)

이 강의에서는 다양한 지표들, 핵심 웹 척도가 SEO에 미치는 영향, 그리고 사용자 경험에 대한 중요성을 살펴볼 것입니다.

핵심 웹 척도를 측정할 때 세 가지 다른 값이 있습니다: "<span className="text-blue-600 font-bold">Good(좋음)</span>", "<span className="font-bold underline">Need Improvement(개선 필요)</span>", "<span className="text-red-600 font-bold">Poor(나쁨)</span>". 이러한 값은 측정되는 척도에 따라 다릅니다.

![Web Vitals](https://nextjs.org/static/images/learn/seo/vitals-light.png)

핵심 웹 척도에는 두 가지 방법으로 접근할 수 있습니다:
...
그러나 <span className="text-blue-600 font-bold">Total Blocking Time (TBT)</span> 지표는 실<span className="font-semibold bg-yellow-100">험실에서 측정 가능</span>하며 상호 작용에 영향을 미치는 문제를 포착합니다.
...
세 가지 지표는 반드시 동등하게 평가되지 않습니다. [Lighthouse](https://developer.chrome.com/docs/lighthouse/overview/)에서는 핵심 웹 척도 각각에 다른 가중치가 부여됩니다:
...
*이것은 FID 첫 입력 지연과 유사합니다.

<Callout type="warning">
참고: 모든 핵심 웹 척도가 좋은 범위에 있는 모든 페이지에 대한 Google 순위 영향은 개별 핵심 웹 척도 점수와 관계없이 동일할 것입니다.
</Callout>

### UX 영향
핵심 웹 척도 주위의 대부분의 대화는 주로 SEO에 중점을 둡니다.

핵심 웹 척도가 페이지 경험과 검색 순위의 개선을 측정하고 추진하기 위한 이니셔티브로 설계되었다는 것은 사실이지만, 이러한 변화로부터 최종적으로 혜택을 받는 것은 사용자들입니다.

핵심 웹 척도는 최상의 페이지 경험을 추구하는 데 도움이 됩니다. 2012년 [아마존에서 수행한 연구](https://www.fastcompany.com/1825005/how-one-second-could-cost-amazon-16-billion-sales)에 따르면, 로드 시간이 1초 증가하면 회사에 16억 달러의 비용이 발생할 수 있습니다. 이러한 연구는 훌륭한 페이지 경험과 빠른 웹사이트의 중요성을 보여주며, 이러한 것들은 핵심 웹 척도가 도움을 줍니다.
