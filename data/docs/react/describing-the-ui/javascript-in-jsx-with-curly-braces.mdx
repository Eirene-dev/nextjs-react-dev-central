...
title: JSX 내 자바스크립트
date: '2023-04-05'
summary: JSX를 사용하여 자바스크립트와 HTML을 효과적으로 통합하는 방법에 대한 소개입니다.
---
...
하지만 동적으로 src 또는 alt 텍스트를 지정하려면 `"`와 `"` 대신 `{`와 `}`를 사용하여 자바스크립트 값을 사용할 수 있습니다.
...
JSX에서 `{{`와 `}}`를 볼 때는 JSX 중괄호 안에 있는 자바스크립트 객체일 뿐이라는 것을 알아두세요!
...
- `{{`와 `}}`는 특별한 구문이 아니라 JSX 중괄호 안에 포함된 자바스크립트 객체입니다.
