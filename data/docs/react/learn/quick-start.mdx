...
### 컴포넌트 생성 및 중첩 방법
React 앱은 크거나 작은 컴포넌트로 구성되며, 이 컴포넌트는 JSX 문법을 사용해 마크업을 반환합니다.
</Card>

<Card href="#jsx를-사용한-마크업-작성">
### 마크업 및 스타일 추가하기
React는 JSX 마크업 구문을 사용합니다. JSX는 모든 태그를 닫아야 하며, `className`으로 CSS 클래스를 지정합니다.
</Card>

<Card href="#데이터-표시">
### 데이터 표시하기
React에서는 JSX 내에 중괄호를 사용해 JavaScript 표현식을 삽입하며, 이를 통해 동적 데이터를 표시합니다.
</Card>

<Card href="#조건부-렌더링">
### 조건 렌더링하기
React에는 조건부 렌더링을 위한 별도의 구문이 없습니다. 대신, JavaScript의 `if-else`나 삼항 연산자로 컴포넌트나 요소의 렌더링을 결정합니다.
</Card>

<Card href="#목록-렌더링">
### 목록 렌더링하기 
React는 데이터 배열을 매핑해 JSX 요소 목록을 생성하며, `key` 속성으로 각 요소를 고유하게 식별합니다.
</Card>

<Card href="#이벤트에-응답하기">
### 이벤트 응답하기
React는 사용자 이벤트에 응답하기 위해 컴포넌트에서 이벤트 핸들러 함수를 정의합니다.
</Card>

<Card href="#화면-업데이트">
###  화면 업데이트하기
React의 `useState` 훅으로 컴포넌트의 로컬 상태를 관리하며, 상태 변경 시 컴포넌트가 다시 렌더링됩니다.
</Card>
...
React 앱은 컴포넌트로 구성됩니다. 컴포넌트는 <span className="text-red-600 font-bold">컴포넌트</span>라는 개체로 구성됩니다. 컴포넌트는 JavaScript 클래스나 함수로 이루어져 있으며, <span className="font-bold text-blue-600">자체적인 로직과 외관을 가진 UI(사용자 인터페이스)의 한 부분</span>입니다. 컴포넌트는 버튼과 같이 작을 수도 있고 전체 페이지와 같이 클 수도 있습니다.

React 컴포넌트는 <span className="font-bold bg-yellow-100">마크업을 반환하는 JavaScript 함수</span>입니다:
...
`<MyButton />`이 <span className="font-bold underline">대문자로 시작함</span>에 유의하세요. 이렇게 하면 React 컴포넌트라는 것을 알 수 있습니다. React 컴포넌트 이름은 항상 대문자로 시작해야 하며, <span className="font-bold underline">HTML 태그는 소문자</span>여야 합니다.
...
<details className="bg-gray-100 p-2 rounded-md">
  <summary>자세히 알아보기: **`export default`**</summary>
...
<details className="bg-gray-100 p-2 rounded-md">
  <summary>자세히 알아보기: **React 이벤트**</summary>
...
- **대소문자 구분**: <span className="text-red-600 font-bold">React 이벤트</span>는 
...
- **이벤트 객체**: <span className="text-red-600 font-bold">이벤트를 처리하는 함수를 정의</span>할 때, React는 해당 함수에 <span className="font-bold bg-yellow-100">이벤트 객체를 전달</span>합니다. 이 이벤트 객체는 네이티브 HTML 이벤트 객체와 동일한 속성과 메서드를 갖지만, 모든 브라우저에서 동일하게 작동합니다 (React는 브라우저의 네이티브 이벤트를 SyntheticEvent로 감싸서 사용합니다).
...
- **`this`에 대한 접근**: <span className="font-semibold underline">JavaScript 클래스에서는 기본적으로 메서드가 바인딩[^1]되지 않습니다.</span> `this.handleClick`을 바인딩하지 않고 `onClick`에 전달하면 함수가 호출될 때 `this`는 `undefined`가 됩니다. 이는 React에 특정한 동작이 아니라 JavaScript에서 함수가 작동하는 방식의 일부입니다.
...
<details className="bg-gray-100 p-2 rounded-md">
  <summary>자세히 알아보기: **React에서 "state"라는 용어**</summary>

React에서 "state"라는 용어는 컴포넌트의 내부 상태를 나타내는데 사용됩니다. 컴포넌트의 "state"는 시간에 따라 변경될 수 있으며, 그 변경은 컴포넌트의 재렌더링을 일으킬 수 있습니다.

React에서 "state"라는 용어를 사용하는 이유는 다음과 같습니다:

1. **의미적 명확성**: 상태(state)는 변화할 수 있는 것을 나타내며, 그 변화는 특정 조건 또는 이벤트에 따라 발생합니다. 반면, "props"는 부모 컴포넌트로부터 전달되는 고정된 값이므로 변경될 수 없습니다.

2. **프로그래밍 패러다임**: React는 함수형 프로그래밍 패러다임에서 영향을 받았습니다. 함수형 프로그래밍에서 "상태"는 애플리케이션의 데이터와 그 데이터의 변화를 나타내는 중요한 개념입니다.

3. **사용자 인터페이스와의 연관성**: 사용자 인터페이스는 사용자와의 상호작용에 따라 변경될 수 있습니다. 이러한 변경을 추적하고 관리하기 위해 "state"라는 개념이 필요합니다.

4. **명료성**: React 컴포넌트의 "state"와 "props"는 뚜렷한 차이가 있습니다. "state"는 컴포넌트 내부에서 관리되며 변경될 수 있습니다. 반면 "props"는 부모 컴포넌트로부터 받은 값으로, 직접 변경할 수 없습니다.

요약하면, React에서 "state"라는 용어는 컴포넌트의 내부 상태와 그 상태의 변화를 관리하기 위해 사용되며, 이는 React의 주요 개념 중 하나입니다.
</details>
...
<details className="bg-gray-100 p-2 rounded-md">
  <summary>자세히 알아보기: **유용한 React 훅**</summary>
...
<details className="bg-gray-100 p-2 rounded-md">
  <summary>자세히 알아보기: **React에서 "props"라는 용어**</summary>

React에서 "props"라는 용어는 "properties"의 줄임말입니다. "properties"는 객체, 컴포넌트, 함수 등에 대한 설정 또는 특성을 나타내는 일반적인 프로그래밍 용어입니다. React에서 "props"는 부모 컴포넌트로부터 자식 컴포넌트로 정보를 전달하는 데 사용되는 데이터입니다.

React에서 "props"라는 용어를 사용하는 이유는 다음과 같습니다:

1. **정적 특성**: "props"는 변경이 불가능한(immutable) 데이터로서, 컴포넌트가 생성될 때 전달되며 그 후에는 변경되지 않습니다. 이러한 특성은 컴포넌트의 동작을 예측 가능하게 하고 버그를 최소화합니다.

2. **단방향 데이터 흐름**: React의 핵심 원칙 중 하나는 단방향 데이터 흐름입니다. 부모 컴포넌트에서 자식 컴포넌트로 "props"를 통해 데이터가 전달되며, 이는 컴포넌트 간의 상호작용과 정보 흐름을 명확하게 합니다.
...
요약하면, React에서 "props"는 컴포넌트 간의 데이터 전달과 컴포넌트의 재사용성을 위해 사용되는 중요한 개념입니다.
</details>
