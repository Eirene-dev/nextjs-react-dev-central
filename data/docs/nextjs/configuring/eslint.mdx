...
- **Strict 엄격**: Next.js의 기본 ESLint 설정과 함께 더 엄격한 [Core Web Vitals 규칙 세트](#코어-웹-바이탈스)를 포함합니다. ESLint를 처음 설정하는 개발자들에게 추천하는 설정입니다.
...
- 다음 플러그인 중 하나 이상을 이미 설치했습니다(`airbnb` 또는 `react-app`과 같은 다른 설정을 통해 또는 따로 설치):
  ...
  - `jsx-a11y`
  - `import`
- Next.js 내의 Babel이 설정된 방식과 다른 특정 `parserOptions`를 정의했습니다(이는 Babel 설정을 사용자 정의 하지 않는 한 권장되지 않습니다).
