...
Next.js는 서버에서 각 fetch 요청에 대한 [캐싱](#데이터-캐싱) 및 [재검증](#데이터-재검증) 동작을 구성할 수 있도록 기본 [`fetch` 웹 API](https://developer.mozilla.org/docs/Web/API/Fetch_API)를 확장합니다. React는 React 컴포넌트 트리를 렌더링하는 동안 fetch 요청을 자동으로 [메모이즈](/docs/nextjs/data-fetching/patterns#필요한-곳에서-데이터-가져오기)합니다.
...
```ts:"app/api/revalidate/route.ts" switcher
...
`fetch`를 지원하거나 노출하지 않는 서드 파티 라이브러리(예: 데이터베이스, CMS, ORM 클라이언트)를 사용하는 경우, [라우트 세그먼트 설정 옵션](https://nextjs.org/docs/app/api-reference/file-conventions/route-segment-config) 및 React의 `cache` 함수를 사용하여 해당 요청의 캐싱 및 재검증 동작을 구성할 수 있습니다.
...
- React의 `cache` 함수는 데이터 요청을 [메모이제이션](/docs/nextjs/caching/request-memoization)하기 위해 사용됩니다.
...
```tsx:"app/item/layout.tsx" switcher
...
<details>
    <summary>JavaScript</summary>
```jsx:"app/item/layout.js" switcher
import { getItem } from '@/utils/get-item'
