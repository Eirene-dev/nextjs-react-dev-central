...
Next.js 13은 새로운 기능과 규칙을 가진 새로운 [App 라우터](/docs/nextjs/routing)를 도입했습니다. 새로운 라우터는 `app` 디렉터리에서 사용할 수 있으며 `pages` 디렉터리와 함께 존재합니다.

Next.js 13으로 업그레이드하는 것은 새로운 [App 라우터](/docs/nextjs/routing#the-app-router)를 사용하는 것을 **요구하지 않습니다**. 두 디렉터리에서 모두 작동하는 새로운 기능, 예를 들어 업데이트된 [이미지 컴포넌트](#image-컴포넌트), [링크 컴포넌트](#link-컴포넌트), [스크립트 컴포넌트](#script-컴포넌트), [폰트 최적화](#폰트-최적화)와 같은 것을 계속 사용할 수 있습니다.
...
App Router로 이동하는 것은 React의 서버 컴포넌트, Suspense 등 Next.js가 기반으로 하는 기능을 처음 사용하는 경우일 수 있습니다. [특별한 파일](/docs/nextjs/routing#파일-규칙) 및 [레이아웃](/docs/nextjs/routing/pages-and-layouts#레이아웃)과 같은 새로운 Next.js 기능과 결합할 때, 마이그레이션은 새로운 개념, 사고 모델 및 행동 변경을 익혀야 합니다.
...
React Context 제공자를 사용하고 있다면, 이들은 [클라이언트 컴포넌트](/docs/nextjs/rendering/client-components)로 이동해야 합니다.
...
`pages` 디렉터리에서는 `next/head` React 컴포넌트를 사용하여 `title` 및 `meta`와 같은 `<head>` HTML 요소를 관리합니다. `app` 디렉터리에서는 `next/head`가 새로운 [내장 SEO 지원](/docs/nextjs/optimizing/metadata)으로 대체됩니다.

**변경 전:**

```tsx:"pages/index.tsx" switcher
...
```tsx:"app/home-page.tsx" switcher
'use client'
...
```tsx:"app/example-client-component.tsx" switcher
'use client'
...
`pages` 디렉터리는 페이지의 데이터를 가져오기 위해 `getServerSideProps`와 `getStaticProps`를 사용합니다. `app` 디렉터리 내에서 이전의 데이터 가져오기 함수들은 `fetch()`와 `async` React 서버 컴포넌트 위에 구축된 [더 간단한 API](/docs/nextjs/data-fetching)로 대체됩니다.
...
`app` 디렉터리에서는 [서버 컴포넌트](/docs/nextjs/rendering/server-components)를 사용하여 React 컴포넌트 내부에 데이터 가져오기를 함께 위치시킬 수 있습니다. 이렇게하면 클라이언트로 보내는 자바스크립트 양을 줄이면서 서버에서 렌더링된 HTML을 유지할 수 있습니다.
...
> **알아두면 좋아요**: 클라이언트에서 외부 API를 호출하기 위해 API 라우트를 이전에 사용한 경우, 이제 [Server Components](/docs/nextjs/rendering/server-components)를 사용하여 데이터를 안전하게 가져올 수 있습니다. [data fetching](/docs/nextjs/data-fetching/fetching-caching-and-revalidating)에 대한 자세한 내용은 여기에서 알아보세요.
