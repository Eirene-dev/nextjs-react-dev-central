...
- `next dev`로 로컬에서 실행할 때, 로컬 스타일시트(전역 또는 CSS 모듈 포함)는 편집 내용이 저장됨에 따라 즉시 변경사항을 반영하기 위해 [Fast Refresh](/docs/architecture/fast-refresh)를 활용합니다.
- `next build`로 제품 환경을 위해 빌드할 때, CSS 파일은 스타일을 검색하기 위해 필요한 네트워크 요청 수를 줄이기 위해 더 적은 수의 축소된 `.css` 파일로 번들됩니다.
- JavaScript를 비활성화하면 제품 환경 빌드(`next start`)에서 스타일이 여전히 로드됩니다. 그러나 [Fast Refresh](/docs/architecture/fast-refresh)를 활성화하기 위해 `next dev`에는 여전히 JavaScript가 필요합니다.