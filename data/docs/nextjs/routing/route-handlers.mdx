...
> **알아두면 좋은 정보**: 라우트 핸들러는 `app` 디렉토리 내에서만 사용 가능합니다. `pages` 디렉토리 내의 [API 라우트](/docs/pages/building-your-application/routing/api-routes)와 동등한 기능을 제공하므로 API 라우트와 라우트 핸들러를 함께 사용할 필요는 **없습니다**.

## 규칙

라우트 핸들러는 `app` 디렉토리 내 [`route.js|ts` 파일](/docs/app/api-reference/file-conventions/route)에 정의됩니다:
...
기본 [Request](https://developer.mozilla.org/docs/Web/API/Request) 및 [Response](https://developer.mozilla.org/docs/Web/API/Response)를 지원하는 것 외에도, Next.js는 고급 사용 사례를 위한 편리한 도우미를 제공하기 위해 [`NextRequest`](/docs/app/api-reference/functions/next-request) 및 [`NextResponse`](/docs/app/api-reference/functions/next-response)로 확장합니다.
...
> **TypeScript 주의**: `Response.json()`은 유효하지만, 기본 TypeScript 타입은 현재 오류를 표시합니다. 대신 타입이 지정된 응답을 위해 [`NextResponse.json()`](/docs/app/api-reference/functions/next-response#json)을 사용할 수 있습니다.
...
- 다른 HTTP 메서드를 사용합니다.
- `cookies`와 `headers`와 같은 [동적 함수](#dynamic-functions)를 사용합니다.
- [세그먼트 설정 옵션](#segment-config-options)을 통해 수동으로 동적 모드를 지정합니다.
...
> **알아두면 좋은 정보**: API 라우트와 마찬가지로, 라우트 핸들러는 폼 제출 처리와 같은 경우에 사용될 수 있습니다. React와 깊게 통합되는 [폼 및 변형 처리](/docs/app/building-your-application/data-fetching/forms-and-mutations)를 위한 새로운 추상화가 진행 중입니다.
...
[`next.revalidate`](/docs/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data) 옵션을 사용하여 [캐시된 데이터를 재검증](/docs/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data)할 수 있습니다:
...
또는 [`revalidate` 세그먼트 설정 옵션](/docs/app/api-reference/file-conventions/route-segment-config#revalidate)을 사용할 수 있습니다:
...
Route 핸들러는 Next.js의 [`cookies`](/docs/app/api-reference/functions/cookies)와 [`headers`](/docs/app/api-reference/functions/headers)와 같은 동적 함수와 함께 사용될 수 있습니다.

#### 쿠키

`next/headers`에서 [`cookies`](/docs/app/api-reference/functions/cookies)를 사용하여 쿠키를 읽을 수 있습니다. 이 서버 함수는 Route 핸들러에서 직접 호출되거나 다른 함수 내에서 중첩될 수 있습니다.
...
대안으로 기본 Web API 위에 추상화를 사용하여 쿠키를 읽을 수 있습니다. ([`NextRequest`](/docs/app/api-reference/functions/next-request)):
...
`next/headers`에서 [`headers`](/docs/app/api-reference/functions/headers)를 사용하여 헤더를 읽을 수 있습니다. 이 서버 함수는 Route 핸들러에서 직접 호출되거나 다른 함수 내에서 중첩될 수 있습니다.
...
대안으로, 헤더를 읽기 위해 기본 Web API 위에 추상화를 사용할 수 있습니다. ([`NextRequest`](/docs/app/api-reference/functions/next-request)):
...
> 계속하기 전에 [라우트 정의하기](/docs/app/building-your-application/routing/defining-routes) 페이지를 읽어보는 것을 권장합니다.
...
라우트 핸들러는 스트리밍을 포함하여 엣지와 Node.js 런타임을 모두 원활하게 지원하기 위해 동형 웹 API를 가지고 있습니다. 라우트 핸들러는 페이지와 레이아웃과 동일한 [라우트 세그먼트 구성](/docs/app/api-reference/file-conventions/route-segment-config)을 사용하므로 일반적으로 요청되는 기능인 [정적으로 재생성된](/docs/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data) 라우트 핸들러를 지원합니다.
...
라우트 핸들러는 페이지와 레이아웃과 동일한 [라우트 세그먼트 구성](/docs/app/api-reference/file-conventions/route-segment-config)을 사용합니다.
