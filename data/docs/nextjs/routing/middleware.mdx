---
title: 미들웨어
date: '2023-09-01 01:11'
summary: 요청이 완료되기 전에 코드를 실행하는 미들웨어의 사용법을 학습하세요.
---
<TOCInline toc={props.toc} asDisclosure />


{/* 이 문서의 내용은 앱과 페이지 라우터 사이에서 공유됩니다. `<PagesOnly>Content</PagesOnly>` 컴포넌트를 사용하여 페이지 라우터에 특정한 내용을 추가할 수 있습니다. 공유되는 내용은 컴포넌트 안에 둘 필요가 없습니다. */}

미들웨어는 요청이 완료되기 전에 코드를 실행할 수 있게 해줍니다. 들어오는 요청을 기반으로 응답을 리라이트, 리디렉션, 요청 또는 응답 헤더 수정, 또는 직접 응답하는 방식으로 수정할 수 있습니다.

미들웨어는 캐시된 컨텐츠와 라우트가 일치되기 전에 실행됩니다. [경로 매칭](#matching-paths)에서 자세한 내용을 확인하세요.
...
1. 응답을 생성하는 라우트([Page](/docs/app/building-your-application/routing/pages-and-layouts) 또는 [Route Handler](/docs/app/building-your-application/routing/route-handlers))로 `rewrite`합니다.
2. `NextResponse`를 직접 반환합니다. [응답 생성](#producing-a-response)을 참조하세요.
...
| `v12.2.0` | 미들웨어는 안정적이며, [업그레이드 가이드](/docs/messages/middleware-upgrade-guide)를 참조하세요.                         |
