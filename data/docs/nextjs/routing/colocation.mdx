...
[routing 폴더 및 파일 규칙](/docs/getting-started/project-structure#app-routing-conventions)을 제외하면 Next.js는 프로젝트 파일의 구성 및 공동 위치에 대한 **특별한 의견**이 없습니다.

이 페이지는 프로젝트를 구성하기 위해 사용할 수 있는 기본 동작과 기능들을 공유합니다.

- [기본적으로 안전한 공동 위치](#safe-colocation-by-default)
- [프로젝트 구성 기능](#project-organization-features)
- [프로젝트 구성 전략](#project-organization-strategies)

## 기본적으로 안전한 공동 위치

`app` 디렉터리에서는 [중첩된 폴더 계층 구조](/docs/app/building-your-application/routing#route-segments)가 라우트 구조를 정의합니다.
...
> - `app` 안에서 프로젝트 파일을 공동 위치시킬 수는 있지만, **반드시 그렇게 할 필요는 없습니다**. 원한다면 [그들을 `app` 디렉터리 밖에 보관할 수 있습니다](#store-project-files-outside-of-app).
...
언더스코어로 폴더명을 접두하는 것으로 개인 폴더를 만들 수 있습니다: `_folderName`

이것은 폴더가 개인적인 구현 세부사항이며 라우팅 시스템에 의해 고려되어서는 안된다는 것을 나타냅니다. 따라서 폴더와 모든 하위 폴더를 라우팅에서 **제외**시킵니다.

!["개인 폴더를 사용하는 폴더 구조 예제"](https://nextjs.org/_next/image?url=%2Fdocs%2Flight%2Fproject-organization-private-folders.png&w=1920&q=75&dpl=dpl_FcsRuVSqhZQjoKTEBMfGGqHxFR53)

`app` 디렉터리의 파일은 [기본적으로 안전하게 공동 위치될 수 있기 때문](#safe-colocation-by-default)에, 개인 폴더는 공동 위치를 위해 필요하지 않습니다. 그러나 다음과 같은 경우 유용할 수 있습니다:
...
> - 개인 폴더를 사용하지 않는다면, 예상치 못한 이름 충돌을 방지하기 위해 Next.js [특별한 파일 규칙](/docs/getting-started/project-structure#routing-files)을 알고 있으면 좋습니다.
...
- [라우트를 그룹별로 조직화하기](/docs/app/building-your-application/routing/route-groups#organize-routes-without-affecting-the-url-path), 예를 들면, 사이트 섹션, 목적, 팀 별로.
- 같은 라우트 세그먼트 레벨에서 중첩 레이아웃 활성화하기:
  - [같은 세그먼트 내에서 다중 중첩 레이아웃 생성, 다중 루트 레이아웃 포함](/docs/app/building-your-application/routing/route-groups#creating-multiple-root-layouts)
  - [공통 세그먼트 내의 라우트 하위 집합에 레이아웃 추가](/docs/app/building-your-application/routing/route-groups#opting-specific-segments-into-a-layout)

### `src` 디렉터리

Next.js는 옵셔널한 [`src` 디렉터리](/docs/app/building-your-application/configuring/src-directory) 안에 애플리케이션 코드(`app` 포함)를 저장하는 것을 지원합니다. 이로써 애플리케이션 코드와 대부분의 프로젝트 설정 파일들을 구분할 수 있게 됩니다.
...
Next.js는 깊게 중첩된 프로젝트 파일 간의 가져오기를 더 쉽게 읽고 유지 관리할 수 있게 해주는 [모듈 경로 별칭](/docs/app/building-your-application/configuring/absolute-imports-and-module-aliases)을 지원합니다.
...
> **알아두면 좋아요**: 아래 예제에서는 `components`와 `lib` 폴더를 일반화된 플레이스홀더로 사용하고 있습니다. 이들의 명명법은 프레임워크의 특별한 의미를 가지고 있지 않으며, 여러분의 프로젝트는 `ui`, `utils`, `hooks`, `styles` 등의 다른 폴더를 사용할 수 있습니다.
