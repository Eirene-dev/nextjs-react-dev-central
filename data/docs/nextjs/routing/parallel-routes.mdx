...
예를 들면, 팀과 분석 페이지를 동시에 렌더링할 수 있습니다.
...
병렬 라우트는 명명된 **슬롯**을 사용하여 생성됩니다. 슬롯은 `@폴더` 규약으로 정의되며, 동일한 레벨의 레이아웃에 prop으로 전달됩니다.
...
[`useSelectedLayoutSegment`](/docs/app/api-reference/functions/use-selected-layout-segment)와 [`useSelectedLayoutSegments`](/docs/app/api-reference/functions/use-selected-layout-segments) 둘 다 `parallelRoutesKey`를 받아들여, 해당 슬롯 내에서 활성 라우트 세그먼트를 읽을 수 있게 합니다.
...
> 모달에 대한 더 많은 정보는 [라우트 가로채기](/docs/app/building-your-application/routing/intercepting-routes) 섹션에서 다룹니다.
