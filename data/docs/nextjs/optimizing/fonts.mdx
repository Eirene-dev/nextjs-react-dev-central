...
[**`next/font`**](/docs/app/api-reference/components/font)는 당신의 폰트(맞춤 폰트 포함)를 자동으로 최적화하고 개선된 프라이버시와 성능을 위해 외부 네트워크 요청을 제거합니다.
...
Google 폰트는 자동으로 [부분 집합화](https://fonts.google.com/knowledge/glossary/subsetting)됩니다. 이로 인해 폰트 파일의 크기가 줄어들고 성능이 향상됩니다. 미리 로드하려는 부분 집합을 정의해야 합니다. [`preload`](/docs/app/api-reference/components/font#preload)가 `true`인 상태에서 부분 집합을 지정하지 않으면 경고가 발생합니다.
...
더 자세한 정보는 [폰트 API 참조](/docs/app/api-reference/components/font)를 참조하세요.
...
export default function 레이아웃({ children }
: { children: React.ReactNode }) {
  return (
...
또한, [CSS 변수](/docs/app/api-reference/components/font#variable)를 생성하고 선호하는 CSS 솔루션과 함께 사용할 수 있습니다:
...
더 자세한 정보는 [폰트 API 레퍼런스](/docs/app/api-reference/components/font)에서 확인하세요.

## Tailwind CSS와 함께 사용하기

`next/font`는 [Tailwind CSS](https://tailwindcss.com/)와 [CSS 변수](/docs/app/api-reference/components/font#css-variables)를 통해 함께 사용될 수 있습니다.
...
마지막으로, CSS 변수를 [Tailwind CSS 설정](/docs/app/building-your-application/styling/tailwind-css#configuring-tailwind)에 추가하세요:
...
- [고유 페이지](/docs/app/building-your-application/routing/pages-and-layouts#pages)인 경우 해당 페이지의 고유 루트에서 미리 로드됩니다.
- [레이아웃](/docs/app/building-your-application/routing/pages-and-layouts#layouts)인 경우 레이아웃으로 감싸진 모든 루트에서 미리 로드됩니다.
- [루트 레이아웃](/docs/app/building-your-application/routing/pages-and-layouts#root-layout-required)인 경우 모든 루트에서 미리 로드됩니다.
