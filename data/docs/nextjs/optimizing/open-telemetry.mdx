...
다음으로 프로젝트의 **루트 디렉토리**에 사용자 지정 [`instrumentation.ts`](/docs/app/building-your-application/optimizing/instrumentation) (또는 `.js`) 파일을 생성합니다 (하나를 사용하는 경우 `src` 폴더 안에):
...
> - [`pageExtensions` 설정 옵션](/docs/app/api-reference/next-config-js/pageExtensions)을 사용하여 접미사를 추가하는 경우, `instrumentation` 파일 이름도 일치하게 업데이트해야 합니다.
...
플랫폼에서 불가능한 경우 [수동 OpenTelemetry 구성](/docs/pages/building-your-application/optimizing/open-telemetry#manual-opentelemetry-configuration)과 함께 사용자 정의 OpenTelemetry exporter를 사용할 수 있습니다.
