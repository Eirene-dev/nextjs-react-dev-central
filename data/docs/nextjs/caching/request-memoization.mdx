...
React는 동일한 URL과 옵션을 가진 Request를 자동으로 **메모이제이션**하기 위해 [`fetch` API](#fetch)를 확장합니다. 이는 React 컴포넌트 트리에서 여러 위치에서 동일한 데이터를 위해 fetch 함수를 호출하더라도 한 번만 실행할 수 있다는 것을 의미합니다.
...
- 루트를 렌더링하는 동안 특정 요청이 처음 호출되면 그 결과는 메모리에 없으므로 캐시 `미스`가 됩니다.
- 따라서 함수가 실행되고 데이터는 외부 소스에서 가져와 메모리에 저장됩니다.
- 동일한 렌더링 패스에서의 요청의 후속 함수 호출은 캐시 `히트`가 되며, 함수를 실행하지 않고도 메모리에서 데이터가 반환됩니다.
...
> - `fetch`가 적절하지 않은 경우 (예: 일부 데이터베이스 클라이언트, CMS 클라이언트 또는 GraphQL 클라이언트), 함수를 메모이제이션하기 위해 [React `cache` 함수](#react-cache-function)를 사용할 수 있습니다.
