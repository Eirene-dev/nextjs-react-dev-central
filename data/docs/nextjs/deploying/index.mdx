...
축하합니다! 여러분은 이제 Next.js 애플리케이션을 배포할 준비가 되었습니다. 이 페이지에서는 [Next.js Build API](#nextjs-build-api)를 사용하여 관리되거나 자체 호스팅으로 배포하는 방법을 보여줍니다.
...
- `.next/server/pages` – 서버에서 사전 렌더링된 HTML과 JavaScript 진입점. `.nft.json` 파일은 [출력 파일 추적](/docs/pages/api-reference/next-config-js/output)이 활성화되었을 때 생성되며 주어진 페이지에 의존하는 모든 파일 경로를 포함합니다.
...
`.next` 내의 모든 JavaScript 코드는 **컴파일되었으며**, 브라우저 번들은 최상의 성능을 달성하고 [모든 현대 브라우저](/docs/architecture/supported-browsers)를 지원하기 위해 **최소화**되었습니다.
...
Next.js 앱의 정적 HTML 내보내기를 원한다면, [정적 HTML 내보내기 문서](/docs/app/building-your-application/deploying/static-exports)의 지침을 따르세요.
...
다음 서비스들은 Next.js를 [`output: 'export'`](/docs/app/building-your-application/deploying/static-exports)를 사용하여 배포하는 것만 지원합니다.

- [GitHub Pages](https://github.com/vercel/next.js/tree/canary/examples/github-pages)

또한, GitHub Actions, Jenkins, AWS CodeBuild, Circle CI, Azure Pipelines 등의 CI/CD 파이프라인을 통해 [`output: 'export'`](/docs/app/building-your-application/deploying/static-exports)의 출력을 어느 정적 호스팅 제공 업체에나 수동으로 배포할 수 있습니다.
...
> **알아두면 좋아요**: 모든 서버리스 제공 업체가 `next start`로부터 [Next.js Build API](#nextjs-build-api)를 구현하지는 않습니다. 지원되는 기능이 무엇인지 제공 업체에게 확인하십시오.
