...
const nextConfig = {
  ...
  // 선택적: 링크 변경 `/me` -> `/me/` 그리고 `/me.html` -> `/me/index.html` 출력
  // trailingSlash: true,
...
결과 컴포넌트는 초기 페이지 로드를 위한 정적 HTML로 렌더링되며, 라우트 간의 클라이언트 탐색을 위한 정적 페이로드로 렌더링됩니다. 서버 컴포넌트를 정적 내보내기로 사용할 때 [동적 서버 함수](#unsupported-features)를 사용하지 않는 한 변경할 필요가 없습니다.
...
[이미지 최적화](/docs/app/building-your-application/optimizing/images)는 `next.config.js`에서 사용자 정의 이미지 로더를 정의함으로써 `next/image`와 함께 정적으로 내보내어 사용할 수 있습니다. 예를 들어, Cloudinary와 같은 서비스를 사용하여 이미지를 최적화할 수 있습니다:
...
- `dynamicParams: true`를 가진 [동적 라우트](/docs/app/building-your-application/routing/dynamic-routes)
- `generateStaticParams()` 없는 [동적 라우트](/docs/app/building-your-application/routing/dynamic-routes)
- 요청에 의존하는 [라우트 핸들러](/docs/app/building-your-application/routing/route-handlers)
- [쿠키](/docs/app/api-reference/functions/cookies)
- [재작성](/docs/app/api-reference/next-config-js/rewrites)
- [리디렉션](/docs/app/api-reference/next-config-js/redirects)
- [헤더](/docs/app/api-reference/next-config-js/headers)
- [미들웨어](/docs/app/building-your-application/routing/middleware)
- [증분 정적 재생성](/docs/app/building-your-application/data-fetching/fetching-caching-and-revalidating)
- 기본 `loader`와 함께 사용되는 [이미지 최적화](/docs/app/building-your-application/optimizing/images)
- [드래프트 모드](/docs/app/building-your-application/configuring/draft-mode)

이러한 기능 중 어느 것을 `next dev`와 함께 사용하려고 시도하면 루트 레이아웃의 [`dynamic`](/docs/app/api-reference/file-conventions/route-segment-config#dynamic) 옵션을 `error`로 설정하는 것과 유사한 오류가 발생합니다.
...
| 버전      | 변경 사항                                                                                                                                        |
| --------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `v13.4.0` | App Router (안정 버전)는 React 서버 컴포넌트 및 라우트 핸들러를 사용하는 강화된 정적 내보내기 지원을 추가합니다.                     |
| `v13.3.0` | `next export`는 폐기되었으며 `"output": "export"`로 대체되었습니다.                                                              |