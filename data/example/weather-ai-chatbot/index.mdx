...
1. 채팅 서비스에 실시간 날씨 연동: 클라이언트에서 날씨 컴포넌트 렌더링 _(작성 예정)_
1. `AI SDK RSC`를 사용하여 서버에서 날씨 컴포넌트 렌더링하여 스트리밍 _(작성 예정)_
1. OpenAI Assistants 기반으로 클라이언트에서 날씨 컴포넌트 렌더링 _(작성 예정)_
1. 서버에서 날씨 컴포넌트 렌더링하여 스트리밍하는 Assistants _(작성 예정)_
</Callout>
...
{/* 1. [클라언트 렌더링: useChat 훅과 라우트 핸들러에서 streamText 사용](/example/weather-ai-chatbot/client-rendering)
1. [서버 렌더링: useActions 훅과 서버 액션에서 generateText 사용](/example/weather-ai-chatbot/server-rendering)
1. [서버 렌더링(Assistants): useActions 훅과 서버 액션에서 createStreamableUI 사용](/example/weather-ai-chatbot/assistants-server-rendering)
1. [클라이언트 렌더링(Assistants): useAssistant 훅과 라우트 핸들러에서 AssistantResponse 사용](/example/weather-ai-chatbot/assistants-client-rendering) */}
