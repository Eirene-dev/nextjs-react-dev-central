...
export const SAMPLE_QA = {
  '배터리 얼마나 가나요?': { found: true, answer: '일반 사용 기준 약 18시간 지속되며, 45분 충전으로 50%까지 채워집니다.', source_ids: ['p2'] },
  '영상 출력 되는 포트는?': { found: true, answer: 'USB-C 2개가 모두 영상 출력을 지원합니다.', source_ids: ['p3'] },
  '보증 기간은?': { found: true, answer: '기본 2년이고 정품 등록 시 1년이 추가되며, 배터리는 별도 1년 보증입니다.', source_ids: ['p5'] },
  '방수 등급이 어떻게 되나요?': { found: false, answer: '', source_ids: [] }, // ★ 본문에 없음 → '없다'
}
...
export function runSample(question, { onAnswer, delay = 700 }) {
  return new Promise((resolve) => {
    setTimeout(() => {
      onAnswer(SAMPLE_QA[question] || { found: false, answer: '', source_ids: [] })
      resolve()
    }, delay)
  })
}
