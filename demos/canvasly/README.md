...
- ★ **컴포넌트 레지스트리**(`src/components.jsx`): `COMPONENTS = { type → { normalize(raw), render(props) } }`. 현재 5종 — **card · timeline · checklist · table · stat**. 타입 추가 = 레지스트리 한 항목 + `gemini.js` `COMPONENT_TYPES` 한 줄(App은 lookup만, if-분기 없음).
