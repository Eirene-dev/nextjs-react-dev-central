...
export const BYOAI_PRESETS: { key: ByoaiPreset; label: string; prompt: string }[] = [
  {
    ...
    label: '교정',
    prompt:
      "아래 글의 맞춤법·띄어쓰기·문법·오타만 교정해 주세요. 문체나 표현은 바꾸지 말고, 고친 부분은 '원래 → 고침 (이유)' 형태로 알려주세요. 다 본 뒤 애매한 곳이 있으면 제게 물어봐 주세요.",
  },
  {
    key: 'structure',
    label: '구조분석',
    prompt:
      '아래 글의 논증 구조를 깊이 비평해 주세요. 주제(주장)가 분명한지, 주장→근거→예시의 흐름에 빈틈이나 비약은 없는지, 예상 반론을 충분히 다뤘는지, 결론이 논증에서 자연스럽게 따라 나오는지 짚어 주세요. 약한 부분은 어디를 어떻게 고치면 좋을지 제안하고, 더 볼 곳이 있으면 제게 되물어 주세요.',
  },
  {
    key: 'content',
    label: '내용 논의',
    prompt:
      '아래 글을 진지한 대화 상대로서 읽고 내용에 대해 토론해 주세요. 어디에 동의하고 어디에 동의하지 않는지, 빠진 관점이나 더 강한 반례는 없는지 솔직하게 말해 주세요. 가능하면 철학적 시각이나 유발 하라리 같은 사상가라면 이 주제를 어떻게 볼지도 한 관점으로 제시해 주세요. 칭찬보다 생각을 밀어붙이는 쪽으로, 한두 가지씩 던지고 제 반응을 기다려 주세요.',
  },
  {
    key: 'voice',
    label: '필체 논의',
    prompt:
      '아래 글의 문체(필체)를 분석하고 함께 이야기해 주세요. 지금 글의 목소리·리듬·문장 길이·어조의 특징과 강점·약점을 짚어 주세요. 그리고 유발 하라리처럼 큰 그림을 쉬운 문장으로 풀어내는 식, 또는 다른 유명 저자의 문체를 이 글에 적용한다면 무엇이 좋아지고 무엇을 잃을지 구체적으로 논의해 주세요. 원하면 한 단락을 그 문체로 다시 써서 비교해 주셔도 좋습니다.',
  },
  {
    key: 'thinking',
    label: '사고방식 논의',
    prompt:
      '아래 글에 담긴 사고 방식을 함께 들여다봐 주세요. 제가 어떻게 전제를 세우고 근거를 잇고 결론에 이르는지, 그 사고 패턴과 약점(성급한 일반화, 빠진 가정 등)을 짚어 주세요. 그리고 특정 철학적 사고법(예: 소크라테스식 질문, 변증법)이나 유발 하라리 같은 사상가의 사고 방식을 이 주제에 적용하면 어떻게 달리 접근하게 될지 논의해 주세요. 한두 가지씩 묻고 제 답을 기다린 뒤 더 깊이 들어가 주세요.',
  },
