...
import { allLevelups, allAuthors } from 'contentlayer/generated'
import type { Authors, Levelup } from 'contentlayer/generated'
...
export const generateStaticParams = async () => {
  const paths = allLevelups.map((p) => ({ slug: p.slug.split('/') }))
...
export async function generateMetadata({
  ...
  const post = allLevelups.find((p) => p.slug === slug)
...
export default async function Page({ params }: { params: { slug: string[] } }) {
  ...
  const sortedPosts = sortPosts(allLevelups) as Levelup[]
  const postIndex = sortedPosts.findIndex((p) => p.slug === slug)
  const prev = coreContent(sortedPosts[postIndex + 1])
  const next = coreContent(sortedPosts[postIndex - 1])
  const post = sortedPosts.find((p) => p.slug === slug) as Levelup
  const authorList = post?.authors || ['default']
  const authorDetails = authorList.map((author) => {
    const authorResults = allAuthors.find((p) => p.slug === author)
    return coreContent(authorResults as Authors)
