...
export default function DocsLayout({ children }: DocsLayoutProps) {
  ...
  return (
    <div className="flex-1 lg:grid lg:grid-cols-[220px_1fr] lg:gap-4">
      <button className="flex items-center bg-gray-100 lg:hidden" onClick={onToggleNav}>
        {navShow ? (
          <Icons.leftClose className="align-middle" />
        ) : (
          <Icons.leftOpen className="align-middle" />
        )}
        <span className="ml-2">{navShow ? '목차 닫기' : '예제 목차'}</span>
      </button>
      <aside
